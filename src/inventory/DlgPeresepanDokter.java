...
public final class DlgPeresepanDokter extends javax.swing.JDialog {
    ...
    private double[] jumlah,harga,beli,stok,kapasitas,p1,p2,stokgudang,stokranap,stokigd;
    ...
    public DlgPeresepanDokter(java.awt.Frame parent, boolean modal) {
        ...
        tabModeResep=new DefaultTableModel(null,new Object[]{
                ...
                "Harga(Rp)","Jenis Obat","Aturan Pakai","I.F.","H.Beli","Stok Ralan","Stok Gudang","Stok Ranap","Stok IGD"
            }){
...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
    ...
    */
    ...
    private void validasiResep(){
        r = tbResep.getSelectedRow();
        if(tbResep.getSelectedRow() != -1){
            try {
                if(Double.parseDouble(tbResep.getValueAt(tbResep.getSelectedRow(), 1).toString())>0){
                    if(Double.parseDouble(tabModeResep.getValueAt(r,1).toString())>Double.parseDouble(tabModeResep.getValueAt(r,11).toString())){
                        tabModeResep.setValueAt("", r, 1);
                        JOptionPane.showMessageDialog(rootPane,"Maaf, stok obat "+ tabModeResep.getValueAt(r, 3).toString() +" tidak mencukupi...!!!!"); 
                    }
                }  
            } catch (Exception e) {
                tabModeResep.setValueAt("", r, 1);
    ...
    private void validasiRacikan(){
        row2=tabModeDetailResepRacikan.getRowCount();
        for(r=0;r<row2;r++){ 
            try {
                if(Double.parseDouble(tabModeDetailResepRacikan.getValueAt(r, 13).toString())>0){
                    if(Double.parseDouble(tabModeDetailResepRacikan.getValueAt(r,13).toString())>Double.parseDouble(tabModeDetailResepRacikan.getValueAt(r,7).toString())){
                        tabModeDetailResepRacikan.setValueAt(0, r, 13);
                        JOptionPane.showMessageDialog(null,"Maaf, stok obat "+ tabModeDetailResepRacikan.getValueAt(r, 2).toString() +" tidak mencukupi...!!!!"); 
                    }   
                } 
            } catch (Exception e) {
                tabModeDetailResepRacikan.setValueAt(0, r, 13);
        ...
        try {
            if(kenaikan>0){
                ...
                try{
                    ...
                    rsobat=psresepasuransi.executeQuery();
                    while(rsobat.next()){
                    Double stokgudang = cariStok(rsobat.getString("kode_brng"), "B0002");
                    Double stokranap = cariStok(rsobat.getString("kode_brng"), "B0001");
                    Double stokigd = cariStok(rsobat.getString("kode_brng"), "B0018");
                        tabModeResep.addRow(new Object[] {false,"",rsobat.getString("kode_brng"),rsobat.getString("nama_brng"),
                           rsobat.getString("kode_sat"),rsobat.getString("letak_barang"),Valid.roundUp(rsobat.getDouble("harga"),100),
                           rsobat.getString("nama"),"",rsobat.getString("nama_industri"),
                           rsobat.getDouble("h_beli"),rsobat.getDouble("stok"),
                           stokgudang,stokranap,stokigd
                        });          
                    }  
                }catch(Exception e){
                    System.out.println("Notifikasi : "+e);
                    ...
                    Double stokigd = cariStok(rsobat.getString("kode_brng"), "B0018");
                        if(Jeniskelas.getSelectedItem().equals("Karyawan")){
                            tabModeResep.addRow(new Object[] {false,"",rsobat.getString("kode_brng"),rsobat.getString("nama_brng"),
                               rsobat.getString("kode_sat"),rsobat.getString("letak_barang"),Valid.roundUp(rsobat.getDouble("karyawan"),100),
                               rsobat.getString("nama"),"",rsobat.getString("nama_industri"),
                               rsobat.getDouble("h_beli"),rsobat.getDouble("stok"),stokgudang,stokranap,stokigd
                            });
                        }else if(Jeniskelas.getSelectedItem().equals("Rawat Jalan")){
                            tabModeResep.addRow(new Object[] {false,"",rsobat.getString("kode_brng"),rsobat.getString("nama_brng"),
                               rsobat.getString("kode_sat"),rsobat.getString("letak_barang"),Valid.roundUp(rsobat.getDouble("ralan"),100),
