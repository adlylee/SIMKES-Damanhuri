...
public class DlgFrekuensiPenyakitRanapPerDokter extends javax.swing.JDialog {
    ...
    private String diagnosa = "", dokternya;
     ...
     * @param modal
     */
    public DlgFrekuensiPenyakitRanapPerDokter(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        TableRowSorter<TableModel> sorter1 = new TableRowSorter<TableModel>(tbDokter2.getModel());
        tbDokter2.setRowSorter(sorter1);
        List<RowSorter.SortKey> sortKeys1 = new ArrayList<>(25);
        sortKeys1.add(new RowSorter.SortKey(7, SortOrder.DESCENDING));
        sortKeys1.add(new RowSorter.SortKey(0, SortOrder.DESCENDING));
...
//                kddokter.requestFocus();
    ...
    private void initComponents() {
        ...
        jComboBox2.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Dalam", "Saraf", "Bedah", "Jiwa", "Mata", "Obgyn", "Anak", "THT", "Orthopedi", "Gigi", "Bedah Saraf", "Kulit & Kelamin", "Psikolog", "Fisioterapi" }));
        jComboBox2.setName("jComboBox2"); // NOI18N
        panelisi1.add(jComboBox2);
        ...
        label19.setBounds(430, 10, 75, 23);
        ...
        BtnSeek3.setBounds(830, 10, 28, 23);

        jLabel1.setFont(new java.awt.Font("Lucida Grande", 0, 11)); // NOI18N
        jLabel1.setText("Tampil :");
        jLabel1.setName("jLabel1"); // NOI18N
        FormInput.add(jLabel1);
        jLabel1.setBounds(30, 45, 42, 14);

        jComboBox1.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "10", "Semua" }));
        jComboBox1.setName("jComboBox1"); // NOI18N
        FormInput.add(jComboBox1);
        jComboBox1.setBounds(80, 40, 97, 27);
...
//        nmdokter.setText("");
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgFrekuensiPenyakitRanapPerDokter dialog = new DlgFrekuensiPenyakitRanapPerDokter(new javax.swing.JFrame(), true);
    ...
    private void tampil() {
        ...
        try {
            switch (jComboBox2.getSelectedItem().toString()) {
                case "Dalam":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0013'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Saraf":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0007'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Bedah":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0006'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Jiwa":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0021'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Mata":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0010'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Obgyn":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0012'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Anak":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0011'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "THT":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0009'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Orthopedi":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0005'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Gigi":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0015'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Bedah Saraf":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0024'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Kulit & Kelamin":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0014'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Psikolog":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0018'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;
                case "Fisioterapi":
                    dokternya = Sequel.cariStringArray("SELECT kd_dokter FROM dokter WHERE kd_sps = 'S0001'");
                    dokternya = Sequel.buangChar(dokternya);
                    dokternya = "(" + dokternya + ")";
                    break;

            }
            String query = "select penyakit.kd_penyakit,SUBSTRING(penyakit.nm_penyakit,1,80) as penyakit , COUNT(penyakit.kd_penyakit) as jml from penyakit inner join diagnosa_pasien inner join reg_periksa "
                    + "inner join pasien inner join kamar_inap inner join kamar inner join bangsal inner join dpjp_ranap inner join penjab "
                    ...
                    + "and kamar_inap.kd_kamar=kamar.kd_kamar and kamar.kd_bangsal=bangsal.kd_bangsal and reg_periksa.no_rawat=dpjp_ranap.no_rawat "
                    + "where diagnosa_pasien.status='Ranap' and diagnosa_pasien.prioritas='1' and dpjp_ranap.kd_dokter IN " + dokternya + " and  reg_periksa.tgl_registrasi between ? and ? and bangsal.nm_bangsal like ? and penjab.png_jawab like ? and (penyakit.kd_penyakit like ? or penyakit.nm_penyakit like ?) "
                    + "group by penyakit.kd_penyakit order by jml";
    ...
    private void tampil2() {
        ...
        try {
            String query = "select penyakit.kd_penyakit,SUBSTRING(penyakit.nm_penyakit,1,80) as penyakit, COUNT(penyakit.kd_penyakit) as jml from penyakit inner join diagnosa_pasien inner join reg_periksa "
                    ...
                    + "where kamar_inap.stts_pulang<>'Pindah Kamar' and diagnosa_pasien.status='Ranap' and diagnosa_pasien.prioritas='1' and kamar_inap.tgl_keluar between ? and ? and bangsal.nm_bangsal like ? and dokter.nm_dokter like ? and penjab.png_jawab like ? and (penyakit.kd_penyakit like ? or penyakit.nm_penyakit like ?) "
                    + "group by penyakit.kd_penyakit order by jml";
            if (jComboBox1.getSelectedItem() == "10") {
                query += " DESC limit 10 ";
            }
